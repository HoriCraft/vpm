...
function loadPersonalKey(){
 ...
 if(result.state==='ready'){
  ...
  $('key-hint').textContent='製品版のテスター用キーは個別DMでお渡しします。受け取ったキー付きリンクから開くと、このボタンでコピーできます。Katachiaの利用には有効な解除キーが必要です。';
 }
}
async function start(){
 ...
 try{
  ...
  $('connection').textContent='リポジトリはどなたでも追加できます。Katachiaの利用には、配布者から受け取った有効なキーを登録してください。';
